...
jenkins_initialize() {
  ...
  # Remove existed PV data and reinit jenkins
  if is_boolean_yes "$JENKINS_FIX_K8S_PV"; then
    rm -rf $JENKINS_HOME/*
  fi

  if is_mounted_dir_empty "$JENKINS_HOME" "$JENKINS_SKIP_CHECK_EMPTY_DIRS"; then
