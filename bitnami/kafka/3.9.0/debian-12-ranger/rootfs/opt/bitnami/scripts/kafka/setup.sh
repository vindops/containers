...
# Setup ranger-kafka-plugin

POLICY_CACHE_FILE_PATH=/etc/ranger/$KAFKA_RANGER_REPO_NAME/policycache
CREDENTIAL_PROVIDER_FILE=/etc/ranger/$KAFKA_RANGER_REPO_NAME/cred.jceks

if is_boolean_yes "$KAFKA_RANGER_ENABLED"; then
  if am_i_root; then
    /opt/bitnami/ranger-kafka/enable-kafka-plugin.sh
    kafka_ranger_authorizer=org.apache.ranger.authorization.kafka.authorizer.RangerKafkaAuthorizer
    kafka_server_conf_set "authorizer.class.name" "$kafka_ranger_authorizer"

    # kafka_common_conf_set "${KAFKA_CONF_DIR}/log4j.properties" "log4j.logger.ranger" "$KAFKA_RANGER_LOG_LEVEL"
    # kafka_common_conf_set "${KAFKA_CONF_DIR}/log4j.properties" "log4j.logger.org.apache.ranger" "$KAFKA_RANGER_LOG_LEVEL"
    kafka_common_conf_set "${KAFKA_CONF_DIR}/log4j.properties" "log4j.logger.org.apache.ranger.authorization.kafka.authorizer" "$KAFKA_RANGER_LOG_LEVEL"
    kafka_common_conf_set "${KAFKA_CONF_DIR}/log4j.properties" "log4j.logger.org.apache.ranger.audit.provider" "$KAFKA_RANGER_LOG_LEVEL"

    export CLASSPATH=${KAFKA_CONF_DIR}

    if is_boolean_yes "$BITNAMI_DEBUG"; then
      if [[ -e $POLICY_CACHE_FILE_PATH ]]; then
        debug "POLICY_CACHE_FILE_PATH is existed"
        ls -al $POLICY_CACHE_FILE_PATH
      else
        debug "POLICY_CACHE_FILE_PATH is not existed"
      fi

      if [[ -e $CREDENTIAL_PROVIDER_FILE ]]; then
        debug "CREDENTIAL_PROVIDER_FILE is existed"
        ls -al $CREDENTIAL_PROVIDER_FILE
      else
        debug "CREDENTIAL_PROVIDER_FILE is not existed"
      fi

      for f in $KAFKA_CONF_DIR/*.xml; do
        debug "Print $f content"
        cat $f
      done

      debug "Print ${KAFKA_CONF_DIR}/log4j.properties content"
      cat ${KAFKA_CONF_DIR}/log4j.properties

      debug "CLASSPATH=$CLASSPATH"
    fi
  else
    error "To enable Ranger for Kafka, the container must be run as root"
    exit 1
  fi
fi
