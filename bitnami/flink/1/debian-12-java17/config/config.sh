...
constructFlinkClassPath() {
  ...
  if [[ "$FLINK_DIST" == "" ]]; then
    (echo >&2 "[ERROR] Flink distribution jar not found in $FLINK_LIB_DIR.")
    exit 1
  elif [[ "$FLINK_DIST_COUNT" -gt 1 ]]; then
    (echo >&2 "[ERROR] Multiple flink-dist*.jar found in $FLINK_LIB_DIR. Please resolve.")
    exit 1
  fi
...
findSqlGatewayJar() {
  ...
  # as the classpath and exit function with empty classpath to force process failure
  if [[ "$SQL_GATEWAY" == "" ]]; then
    (echo >&2 "[ERROR] Flink sql gateway jar not found in $FLINK_OPT_DIR.")
    exit 1
  elif [[ "$SQL_GATEWAY_COUNT" -gt 1 ]]; then
    (echo >&2 "[ERROR] Multiple flink-sql-gateway*.jar found in $FLINK_OPT_DIR. Please resolve.")
    exit 1
  fi
...
findFlinkPythonJar() {
  ...
  if [[ "$FLINK_PYTHON" == "" ]]; then
    ...
    (echo >&2 "[ERROR] Multiple flink-python*.jar found in $FLINK_OPT_DIR. Please resolve.")
    exit 1
  fi
