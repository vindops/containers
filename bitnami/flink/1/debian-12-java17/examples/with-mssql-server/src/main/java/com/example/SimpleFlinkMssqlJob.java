...
public class SimpleFlinkMssqlJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        LOG.info("Creating table in SQL Server: {}", jdbcUrl);
        // Tạo bảng trong SQL Server
        createTableInSqlServer(jdbcUrl, username, password);
    ...
    private static void createTableInSqlServer(String jdbcUrl, String username, String password) {
        String createTableSql = "IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'test_table') " +
                               "CREATE TABLE test_table (test_column INT)";

        try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password);
            Statement stmt = conn.createStatement()) {
            stmt.executeUpdate(createTableSql);
            LOG.info("Table test_table created or already exists.");
